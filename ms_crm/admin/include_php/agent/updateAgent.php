...
$strCurrentComp = "Milestone";
$strWorkPhone 	= "";
$strWorkFax 	= "";

$strFirstName 	= addslashes($_GET['firstName']);
$strSurname 	= addslashes($_GET['surname']);
$strPassword 	= addslashes($_GET['password']);
$strAccessLevel = addslashes($_GET['accessLevel']);
$strAccessStatus= addslashes($_GET['accessStatus']);
...
$sql="INSERT INTO `crmdatabase`.`agent` (`agent_ID`, `ag_fName`, `ag_lName`, `ag_password`, `ag_levelAccess`, `ag_logAttempts`, `contact_id`) VALUES 
	  (NULL, '$strFirstName', '$strSurname', '$strPassword', '$strAccessLevel', '$strAccessStatus', $contactID);";
mysql_query($sql)or die('<br /> Query failed'.mysql_error());
echo "Insert Agent Success";
mysql_close($db_con);
?>