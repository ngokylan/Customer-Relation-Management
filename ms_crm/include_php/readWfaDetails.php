...
<?php
	...
	if(!$db) {
		...
		if(isset($_POST['queryString']) && isset($_POST['tempid'])) {
			$canID = $db->real_escape_string($_POST['queryString']);			
			$tempid = $db->real_escape_string($_POST['tempid']);
			if(strlen($canID) >0) {				
				$query = $db->query("
			
				 SELECT wa.wfa_ID, c.can_fName, c.can_lName, wa.status_complete, wc.wfa_template_ID, wc.enduration, wc.comment, wt.content, c.candidate_ID, e.email_ID, wt.subject
