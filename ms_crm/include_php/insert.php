...
//4 Get Recordset based on SQL above
$dbRecords = mysql_query($sql2, $db_con ) 
		Or die('Query failed:'.mysql_error());

//5 Loop through records for: $state, $state_code, $state_name
$state = "";
while( $arrRecords2 = mysql_fetch_array($dbRecords)) 
{		
...
//load WFA title and id from the database - MINH
$sql3 = 'select * from wfa';

//4 Get Recordset based on SQL above
$dbRecords = mysql_query($sql3, $db_con ) 
		Or die('Query failed:'.mysql_error());

//5 Loop through records for: $state, $state_code, $state_name
$assignWFA = "";
while( $arrRecords3 = mysql_fetch_array($dbRecords)) 
{		
	$wfa_id = $arrRecords3["WFA_ID"];
	$title = $arrRecords3["Title"];
	$assignWFA .= '<option value="'.$wfa_id.'">'.$title.'</option>';
}
...
<form id="insertForm" name="insertForm">
  ...
  <div class="floatRow">																			<!--First Name || Surname-->
    	<label for="txtInsertSkill" class="labelStyleLeft">Skill Set</label>
...
<label for="txtInsertCurComp" class="labelStyleLeft">Company</label>
  ...
  <label for="txtInsertNextAvail" class="labelStyleLeft"
                ...
                onDblClick="helpSummary(this,'<?php echo $hs_nextContact?>', '<?php echo $hs_HnextContact." field"?>');">Next Contact</label>
        ...
        <!--Initial Contact--><!-- Help System: sch09297795 - 05/10/10-->
   	  <label for="dateInsertInitialContact" class="labelStyleRight"
                ...
                onDblClick="helpSummary(this,'<?php echo $hs_iniContact?>', '<?php echo $hs_HiniContact." field"?>');">Initial Contacted</label>
    ...
    <div class="floatRow">									<!--Email || Scoring--><!-- Help System: sch09297795 - 05/10/10-->
        ...
        <label for="cboInsertScoring" class="labelStyleRight" id=""
                ...
                onDblClick="helpSummary(this,'<?php echo $hs_scoring?>', '<?php echo $hs_Hscoring." field"?>');">Rating</label>
        <select name="cboInsertScoring" class="selectNonDate" id="cboInsertScoring">
                ...
                <label for="txtInsertPreComp" class="labelStyleRight" id="insertRelState" onMouseOut="closeHelpSummary();" onMouseOver="helpSummary(this,'<?php echo $hs_status?>', '<?php echo $hs_Hstatus." field"?> ');"  onDblClick="helpSummary(this,'<?php echo $hs_status?>', '<?php echo $hs_Hstatus." field"?>');">Relation Status</label>
    ...
    <div class="floatRow">							<!--Engagement Type || Status--><!-- Help System: sch09297795 - 05/10/10-->
       ...
       <select name="cboInsertCurrEngageType" class="selectNonDate" id="cboInsertCurrEngageType">
        ...
        <label for="txtInsertRate" class="labelStyleLeft"
                ...
                onDblClick="helpSummary(this,'<?php echo $hs_rate?>', '<?php echo $hs_Hrate." field"?>');">Rate Requested</label>
...
<input type="text" class="txtBoxShadow" id="txtInsertRate" name="txtInsertRate" style="width:50px"/>
   		<select name="rateType" class="selectNonDate" id="rateType" style="width:76px">
                    <option value="notSelected"  selected="selected">Select </option>
                    <option value="per Hour">per Hour</option>
                    <option value="per Day">per Day</option>
                    <option value="per Annual">per Annum</option>
         </select>
         <label for="txtInsertPreComp" class="labelStyleRight" id="insertRelState"
        		onMouseOut="closeHelpSummary();" 
                onMouseOver="helpSummary(this,'<?php echo $hs_status?>', '<?php echo $hs_Hstatus." field"?> ');" 
                onDblClick="helpSummary(this,'<?php echo $hs_status?>', '<?php echo $hs_Hstatus." field"?>');">Choose WFA</label>
        <select name="cbAssignWFA" class="selectNonDate" id="cbAssignWFA"><!--==========================================-->
                    <option value="notSelected"  selected="selected">Please select </option>
                    <?php echo $assignWFA;?>
		</select>  
         </div>
    ...
    <div class="AdSearchfloatRowBtns">
        <div id="adSearchHelp">
        	<div id="adSearchHelpInner">
                ...
                <span onClick="openHelp_InsUpdCand();"><img src="image/help_icon.jpg" width="20" alt="Help" title="Help"/></span>
            </div>
        </div>
